...
def transform_event(event):
    ...
    return {
        ...
        "genre": genre,
        "subgenre": subgenre,
        "genre_clean": clean_genre(genre, subgenre),
        "event_type": get_nested_value(classification, ["type", "name"]),
        "event_subtype": event_subtype,
        "family": classification.get("family"),
        "is_festival": detect_festival(event.get("name"), event_subtype),
...
def fetch_events():
    ...
    while page < total_pages:
        ...
        print(f"Descargando MX - pagina {page + 1} de {total_pages}...")
...
def main():
    ...
    df = df.sort_values(by=["event_date", "city_clean", "event_name"])

    df.to_csv(PROCESSED_DATA_PATH, index=False, encoding="utf-8-sig")

    print("\nExtraccion MX terminada.")
